...
ZIP_NAME="LittleSpud-${VERSION_LABEL}.zip"
...
DOWNLOAD_URL="${1:-https://raw.githubusercontent.com/TaterTotterson/Little-Spud-WebUI/main/macos/LittleSpud/releases/${ZIP_NAME}}"
...

{
  ...
  printf '  "url": "%s",\n' "${DOWNLOAD_URL}"
  printf '  "sha256": "%s",\n' "${SHA256}"
  printf '  "notes": "Little Spud macOS update %s."\n' "${VERSION_LABEL}"
  printf '}\n'
} > "${MANIFEST_PATH}"
